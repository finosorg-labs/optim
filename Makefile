...
linux:
	...
	@rm -f $(LINUX_BUILD_DIR)/libfinkit_optim_static_base.a
...
macos:
	...
	@echo "==> Cleaning intermediate build artifacts"
	@rm -f $(MACOS_BUILD_DIR)/libfinkit_optim_static_base.a
...
bench:
	...
	@if [ -f $(LINUX_BUILD_DIR)/benchmarks/optim_benchmarks ]; then \
		$(LINUX_BUILD_DIR)/benchmarks/optim_benchmarks; \
	else \
...
clang-tidy:
	...
	@find optim-c include \( -name '*.c' -o -name '*.h' \) \
		! -name 'platform_win.c' \
...
cppcheck:
	...
	@find optim-c include \( -name '*.c' -o -name '*.h' \) -print | \
...
help:
	@echo "optim Makefile - Build and Test Targets"
	@echo ""
	@echo "Build Targets:"
